        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="new-user-info">
                    <form class="" method="POST" action="{{ route('product.update', $data->id) }}" enctype="multipart/form-data">
                        @csrf
                        <div class="row">
                            <div class="form-group col-md-6">
                                <label class="form-label" for="uname">Nombre de Producto:</label>
                                <input type="text" class="form-control" name="name" id="uname"
                                    value="{{ $data->name }}" placeholder="Ejem: Colchoneta">
                                @if ($errors->has('name'))
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <input type="text" class="form-control" name="description" id="description"
                                    value="{{ $data->description }}" placeholder="Ejem: Colchoneta">
                            ...
                            <div class="form-group col-md-6">
                                <label class="form-label" for="duration">Categoría:</label>
                                ...
                                @if ($errors->has('category_id'))
                                    <span class="text-danger">
                                        <small><em>{{ $errors->first('category_id') }}</em></small>
                                    </span>
                                @endif
                            </div>
                            <div class="form-group col-md-6">
                            ...
                            <div class="form-group col-md-6">
                                <label class="form-label" for="price">Foto de Producto:</label>
                                <input type="file" class="form-control" name="image" id="image">
                                <img class="bg-soft-primary rounded img-fluid avatar-40 mt-3" src="{{ asset($data->image) }}" alt="profile">
                                @if ($errors->has('image'))
                                    <span class="text-danger">
                                        <small><em>{{ $errors->first('image') }}</em></small>
                                    </span>
                                @endif
                            </div>
                        </div>
